...
public class OrderRepository
{
    ...
    public List<OrderSummary> GetClientOrders(int clientId, string searchTerm = null)
    {
        ...
        string sql = @"
            SELECT 
                O.Order_Id, O.Total_Amount, OS.Status_Name, O.Date_Order, R.Name, O.Delivery_Address, O.Status_Id
            FROM 
                ""Order"" O
            JOIN 
        ...
        using (var connection = new SqliteConnection(ConnectionString))
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    string orderSql = @"
                        INSERT INTO ""Order"" (Client_Id, Status_Id, Restaurant_Id, Total_Amount, Delivery_Address)
                        VALUES (@Client_Id, 1, @Restaurant_Id, @Total_Amount, @Delivery_Address);
